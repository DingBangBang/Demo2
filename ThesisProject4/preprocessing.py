...
# 数据导入
df = pd.read_csv('F:\Demo\ComplexDemo\comments_summary.txt', encoding='utf-8', header=0)
# df = pd.read_csv('', encoding='utf-8', header=0)
print('该文档共有%s条评论：' % len(df))
print(df.head())
print(type(df))
# print(df.info(verbose=None, buf=None))

# 1文本去重
'''显示int()参数类型错误，不能是NoValueType
df = df.dropna(axis=0)
...
df2 = df2.replace('此用户没有填写评论', '')  # 将默认评论部分替换为空值
df2 = df2.replace('到货神速 双十一力度太给了', '')
...
c2 = len(df3)
print('短句过滤共删除了%s条评论：' % (c1 - c2))
print(df3[:5])
# df.to_csv('temp_after_filter3.csv', index=False, header=False, encoding='utf-8')
df3.to_csv('temp_after_filter3.txt', index=False, header=False, encoding='utf-8')
# print(df.info())
